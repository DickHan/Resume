...
- JAVA、spring MVC、spring cloud、spring boot、mybatis、Redis、kingbase、mysql、oracle、minio、Docker、K8s/Openshift、GuassDB、MPP、hbase、hive、elasticsearch.
...
用户、组织机构体系构建，开发可视化的元数据管理平台，实现数据共享和数据权限控制。负责设计开发了用户、租户服务，实现了面向不同租户下不同用户的数据权限隔离。

#### 委办局业务系统

针对委办局具体需求，对接数据服务平台，从DM数据超市获取数据，建立ADS层应用数据，并基于应用数据开发业务系统，实现跨局的业务数据应用、分析以及展示。主要负责了后端服务的搭建和功能开发，设计了用户组织体系结构和部分业务逻辑的开发。
